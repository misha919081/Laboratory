...
class Tab_Widget(QWidget):
    ...
    def initUI(self):
        ...
        MatrixSizeChangerW = QSpinBox(self)
        self.column = MatrixSizeChangerW.value()
        ...
        MatrixSizeChangerH = QSpinBox(self)
        self.row = MatrixSizeChangerH.value()
        ...
        determinant = QLabel('Для определения детерминанта матрицы данная программа использует следующую формулу:')
        determinant.setAlignment(Qt.AlignCenter)

        window2Content = QVBoxLayout()
        window2Content.addWidget(title)
        window2Content.addWidget(determinant)
...
class Matrix(QGridLayout):
    ...
    def changeMatrixWidth(self):
        ...
        for i in range(self.rowCount()):
            MatrixExpand.append(QLineEdit())
            MatrixExpand[i].setValidator(validator)
            self.addWidget(MatrixExpand[i], i, test)
